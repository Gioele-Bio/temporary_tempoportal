...
import warnings
import uuid
from ..components import redis_store
...
class DataTableSelectColsAIO(html.Div):
    class ids:
        datatable = lambda aio_id: {
            'component': 'DataTableSelectColsAIO',
    ...
    def filter_sort_page(page_current, page_size, sort_by, filter, store):
        ...
        df = DataTableSelectColsAIO.filter_df(df, filter)
        df = DataTableSelectColsAIO.sort_df(df, sort_by)
        df = DataTableSelectColsAIO.page_df(df, page_current, page_size)
